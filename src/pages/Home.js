    ...
    useEffect(() => {
        (async function () {
            if (window.ethereum) {
                ...
                const provider = new ethers.providers.Web3Provider(window.ethereum);
                const contract = getTodoContract(provider.getSigner());
                const length = parseInt((await contract.numberOfTodo(accounts[0])).toString())
                const arr = [];
    ...
    return (
        <>
            <Container maxWidth="lg">
                ...
                <Box sx={{ m: 3 }} style={{ float: 'right' }}><b>Connected to: </b>{account}</Box>
                ...
                <TodoModal account={account} formData={editData} open={open} setOpen={setOpen}></TodoModal>
            </Container>
        </>
    )
}
